...
- (void)viewDidLoad {
    ...
    //
    UIView *view = [[UIView alloc] initWithFrame:CGRectMake(260, 100, 100, 100)];
            ...
            if (phasset.mediaType == PHAssetMediaTypeVideo) {
                ...
                [manager requestAVAssetForVideo:phasset options:nil resultHandler:^(AVAsset * _Nullable asset, AVAudioMix * _Nullable audioMix, NSDictionary * _Nullable info) {
                    ...
                    dispatch_async(dispatch_get_main_queue(), ^{
                        ...
                        [TopReplayManager compressQuailtyWithInputURL:videoURL outputURL:[NSURL fileURLWithPath:outPath] blockHandler:^(AVAssetExportSession *session) {
...
- (BOOL)videoTimeCheck:(NSDate *)videoDate {
    ...
    //判断是否在5分钟之内录制
    BOOL timeOK = [TopReplayManager GetTimeChange:videoDate];
    return timeOK;
}


#pragma mark - app内播放视频
- (void)configVideoPlayer{
    
    //本地视频路径
    NSString* localFilePath=[[NSBundle mainBundle] pathForResource:@"1582" ofType:@"MP4"];
    NSURL *localVideoUrl = [NSURL fileURLWithPath:localFilePath];
    //网络视频路径
//    NSString *webVideoPath = @"http://api.junqingguanchashi.net/yunpan/bd/c.php?vid=/junqing/1115.mp4";
//    NSURL *webVideoUrl = [NSURL URLWithString:webVideoPath];
        AVPlayer *avPlayer= [AVPlayer playerWithURL:localVideoUrl];
       // player的控制器对象
       ...
       // 设置显示的Frame
       playerViewController.view.frame = CGRectMake(20, 250, 300, 500);;
       // 将播放器控制器添加到当前页面控制器中
